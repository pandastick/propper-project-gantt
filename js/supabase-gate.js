...
function installGate({ supabase, win }) {
  ...
  // CRITICAL ordering: fire ppgantt:slug-ready only AFTER the fetch
  // override is installed.  Listeners (index.html's _loadInitialData
  // gate, the Pull/Push button enable-check) assume both __PPGANTT_SLUG__
  // and the fetch override are ready when this event fires.  Firing
  // earlier — before win.fetch was patched — caused loadManifest() to
  // hit the raw static /api/manifest path and fall through to the
  // Notion-era data/_manifest.json snapshot.
  try {
    const detail = { slug };
